...
/*	F	*/
function m_jdate($format,$timestamp='',$none='',$time_zone='Asia/Tehran',$tr_num='fa'){
 ...
 for($i=0; $i<$sl; $i++){
  ...
  switch($sub){
	...
	case'D':
	$out.=m_jdate_words(array('kh'=>$date[7]),' ');
	break;

	case'f':
	$out.=m_jdate_words(array('ff'=>$j_m),' ');
	break;

	case'F':
	$out.=m_jdate_words(array('mm'=>$j_m),' ');
	break;
	...
	case'J':
	$out.=m_jdate_words(array('rr'=>$j_d),' ');
	break;
	...
	case'l':
	$out.=m_jdate_words(array('rh'=>$date[7]),' ');
	break;
	...
	case'M':
	$out.=m_jdate_words(array('km'=>$j_m),' ');
	break;
	...
	case'p':
	$out.=m_jdate_words(array('mb'=>$j_m),' ');
	break;
	...
	case'q':
	$out.=m_jdate_words(array('sh'=>$j_y),' ');
	break;
	...
	case'r':
	$key=m_jdate_words(array('rh'=>$date[7],'mm'=>$j_m));
	...
	case'v':
	 $out.=m_jdate_words(array('ss'=>($j_y%100)),' ');
	break;

	case'V':
	$out.=m_jdate_words(array('ss'=>$j_y),' ');
	break;
...
function jstrftime($format,$timestamp='',$none='',$time_zone='Asia/Tehran',$tr_num='fa'){
 ...
 for($i=0; $i<$sl; $i++){
  ...
  switch($sub){
	...
	/* Day */
	case'a':
	$out.=m_jdate_words(array('kh'=>$date[6]),' ');
	break;

	case'A':
	$out.=m_jdate_words(array('rh'=>$date[6]),' ');
	break;
	...
	case'c':
	$key=m_jdate_words(array('rh'=>$date[6],'mm'=>$j_m));
  ...
  if($m===''){
   ...
   if($s===''){
    ...
    if($jm===''){
     ...
     $m_jdate=explode('_',m_jdate('Y_j','','',$timezone,'en'));
     if($jd===''){
      list($gy,$gm,$gd)=jalali_to_gregorian($m_jdate[0],$jm,$m_jdate[1]);
      return mktime($h,$m,$s,$gm);
     }else{
      if($jy===''){
       list($gy,$gm,$gd)=jalali_to_gregorian($m_jdate[0],$jm,$jd);
...
function jgetdate($timestamp='',$none='',$timezone='Asia/Tehran',$tn='en'){
 ...
 $m_jdate=explode('_',m_jdate('F_G_i_j_l_n_s_w_Y_z',$ts,'',$timezone,$tn));
 return array(
	'seconds'=>tr_num((int)tr_num($m_jdate[6]),$tn),
	'minutes'=>tr_num((int)tr_num($m_jdate[2]),$tn),
	'hours'=>$m_jdate[1],
	'mday'=>$m_jdate[3],
	'wday'=>$m_jdate[7],
	'mon'=>$m_jdate[5],
...
/*	F	*/
function m_jdate_words($array,$mod=''){
