        ...
        <div class="container">
            <div class="row">
                <div class="col-sm-12 col-md-9">
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="post-image opacity"><img src="{{$post->thumb}}" width="1170" height="382" alt="{{$post->title}}"
                                                                 title="{{$post->title}}"></div>
                            <div class="post-content top-pad-20">
                                <h1 itemprop="name" class="post-title"><a
                                        href="/{{$locale}}{{$post->link_address}}">{{$post->title}}</a></h1>
                                <p>{{$post->excerpt}}</p>
                                <p>{!! $post->content !!}</p>
                            </div>
                            <div class="post-meta">
                            ...
                            <div class="col-md-12 top-pad-20">
                                ...
                                @foreach($post->comments as $item)
                                    <div class="comment-item">
                                        <div class="pull-left author-img"><img class="img-circle"
                                                                               src="{{theme_assets("assets/img/sections/testimonials/1.jpg")}}"
                                                                               width="80" height="80"
                                                                               alt="{{$item->full_name}}"
                                                                               title="{{$item->full_name}}"></div>
                                        <p>{!!  $item->content !!}</p>
                                        <div class="post-meta">
                                            <!-- Author  -->
                                            ...
                                            <!-- Meta Date -->
                                            <span class="time"><i
                                                    class="fa fa-calendar"></i>   <?php $date = \Morilog\Jalali\Jalalian::fromDateTime($item->created_at)->format("Y/m/d") ?>
                                                {{$date}}</span>
                                            <!-- Category -->
                                            <span class="comments pull-right"><i class="fa fa-comments"></i> <a
                                                    href="#">reply</a></span>
                                        </div>
                                    </div>
                                @endforeach
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="input-email form-group">
                                        <input name="email" type="email" id="tm-comment-email"
                                               class="input-email form-control" placeholder="ایمیل*"/>
                                    </div>
                                </div>
                                <div class="col-md-8">
                                    <!-- Field 4 -->
                                    <div class="textarea-message form-group">
                                        <textarea name="content" id="tm-comment-textbox"
                                                  class="textarea-message form-control" placeholder="نظر*"
                                                  rows="4"></textarea>
                                    </div>
                                    <!-- Button -->
                                    <button class="btn btn-default" type="submit">ارسال نظر <i
