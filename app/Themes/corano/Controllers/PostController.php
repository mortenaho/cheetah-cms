...
class PostController extends Controller
{
    ...
    public function portfolioDetailsAjax(Request $request)
    {
        try {
            $id=$request->p_id;
            ...
            if (isset($portfolio->id)) {
                $html =  view()->make("home::portfolio._portfolio_modal", compact('portfolio'))->render();
                return \response()->json(["status" => "true", "html" => $html], 200);
            } else {
                return \response()->json(["status" => "false"], 200);
            }
            ...
            Log::error($exception->getMessage(), ["code" => $exception->getCode(), "line" => $exception->getLine()]);
            return \response()->json(["status" => "false"], 500);
//            return \response()->json(["error" => $exception->getMessage(),"id"=>$id], 500);
        }
    }
