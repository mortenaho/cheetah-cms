...
class PostController extends Controller
{
    ...
    public function portfolio($locale,$id, $slug = null)
    {
        try {
            ...
            //$portfolios = Posts::with("post_meta")->where("type", "=", "portfolio")->where("status", "=", "1")->get();
            $portfolios= Post::with("post_meta")->where("post_type", "portfolio")
                ...
                ->skip(0)->take(2)
                ->get();
            return view("home::portfolio.details", compact('post','portfolios'));
        } catch (\Exception $exception) {
            Log::error($exception->getMessage(), ["code" => $exception->getCode(), "line" => $exception->getLine()]);
            return abort(404, Message404Search);
        }
