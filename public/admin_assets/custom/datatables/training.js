...
// set data to table
    var table = $("#training-table").DataTable({
        ...
        "preDrawCallback": function( settings ) {
            loadingElement('#training-table_wrapper',true);
        },
        'ajax': "/admin/dtAjaxData/training",
        ...
        "columns": [
            ...
            //
            //         var featuredTitle = row.featured == 0 ? "معمولی" : "ویژه";
            //         var featuredClass = row.featured == 0 ? "default" : "success";
            //
            //         return '<span data-id="' + row.id + '" data-sort="' + row.status + '" class="label label-' + statusClass + ' naw3-row-status btn-ladda btn-ladda-spinner" data-spinner-color="#333" data-style="zoom-out"><span class="ladda-label">' + statusTitle + '</span></span>' +'&nbsp'+
            //             '<span data-id="' + row.id + '" data-sort="' + row.featured + '" class="label label-' + featuredClass + ' naw3-row-featured btn-ladda btn-ladda-spinner" data-spinner-color="#333" data-style="zoom-out"><span class="ladda-label">' + featuredTitle + '</span></span>';
            //         },
            //
            // },
            {
            ...
            },
            {
                "mRender": function (data, type, row) {
                    return '<ul class="icons-list">' +
                        '<li class="btn btn-primary btn-icon btn-rounded legitRipple"><a href="/admin/training/' + row.id + '/edit"><i class="icon-pencil7"></i></a></li>' +
        ...
        "drawCallback": function( settings ) {
            ...
            loadingElement('#training-table_wrapper',false);
        }
    });
...
//change Status
    ...
    function changeFeatured(id) {
        var request = $.ajax({
            url: "/admin/training/featured/" + id,
            type: "POST",
            data: {id: id},
            dataType: "json"
        });
...
function changeStatusSwitch(id) {
    console.log(id);
    var loadElement =  $('#training-table_wrapper');
