   ...
   var id=$("[name='type_id']").val();
    var table = $("#attachment-table").DataTable({
        ...
        "preDrawCallback": function( settings ) {
            loadingElement('#attachment-table_wrapper',true);
        },
        'ajax': "/admin/dtAjaxData/attachment/"+id,
            ...
            {"data": "title"},
            {
                "mRender": function (data, type, row) {
                    if(row.file !=null && row.file !=='' ) {
                        return '<a href="' + row.file + '" data-popup="lightbox">مشاهده</a>';
                    }
                    else{
                        return '<a href="#" data-popup="lightbox"></a>';
                    }
                }
            },
            {
                "mRender": function (data, type, row) {
                    if(row.link_address !=null && row.link_address !=='' ) {
                        return '<a target="_blank" href="' + row.link_address + '" data-popup="lightbox">لینک به آدرس</a>';
                    }
                    else{
                        return '<a href="#" data-popup="lightbox"></a>';
                    }
            ...
            },
            {
                "mRender": function (data, type, row) {
                    return '<ul class="icons-list">' +
                        '<li class="btn btn-primary btn-icon btn-rounded legitRipple"><a href="/admin/attachment/' + row.id + '/edit"><i class="icon-pencil7"></i></a></li>' +
        ...
        "drawCallback": function( settings ) {
            ...
            loadingElement('#attachment-table_wrapper',false);
        }
    });
...
//change Status
    ...
    function changeStatus(id) {
        var request = $.ajax({
            url: "/admin/attachment/status/" + id,
            type: "POST",
            data: {id: id},
            dataType: "json"
        });
...
function changeStatusSwitch(id) {
    console.log(id);
    var loadElement =  $('#attachment-table_wrapper');
