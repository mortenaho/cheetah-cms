...
// set data to table
    var table = $("#customers-table").DataTable({
        ...
        "preDrawCallback": function( settings ) {
            loadingElement('#customers-table',true);
        },
        'ajax': "/admin/dtAjaxData/customers",
        "fnCreatedRow": function (nRow, aData, iDataIndex) {
            $(nRow).attr('data-ordering', aData["id"]);
        ...
        "columns": [
            ...
            {
                "mRender": function (data, type, row) {
                    return '  <a href="' + row.avatar + '" data-popup="lightbox">' +
                        '<img src="' + row.avatar + '" alt="" class="img-rounded img-preview">' +
                        '</a>';
            ...
            {"data": "mobile"},
            {
                "mRender": function (data, type, row)
                {
                    ...
                    var featuredChecked = row.is_active === 0 ? "" : " checked ";
            ...
            },
            {
                "mRender": function (data, type, row) {
                    return '<ul class="icons-list">' +
                        '<li class="btn btn-primary btn-icon btn-rounded legitRipple"><a href="/admin/customers/' + row.id + '/edit"><i class="icon-pencil7"></i></a></li>' +
        ...
        "drawCallback": function( settings ) {
            ...
            loadingElement('#customers-table',false);
        }
    });
    ...
    function changeFeatured(id) {
        var request = $.ajax({
            url: "/admin/customers/featured/" + id,
            type: "POST",
            data: {id: id},
            dataType: "json"
        });
...
function changeStatusSwitch(id) {
    console.log(id);
    var loadElement =  $('#customer-table_wrapper');
