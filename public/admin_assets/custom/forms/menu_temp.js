...
$(document).ready(function () {
    ...
    $(document).on("click", ".btn-update-menu-item", function () {
        ...
        //loadingElement(divElement,true);
        var request = $.ajax({
            url: "/admin/menu_temp/menu_edit_title",
            type: "POST",
...
function order_menu_request(data) {
    ...
    // console.log(data);
    var request = $.ajax({
        url: "/admin/menu_temp/order_menu",
        type: "POST",
        data: {data: data},
        dataType: "json"
    });
...
function deleteItem(id, row) {
    ...
    request.done(function (res) {
        ...
        if (res.status === "true") {
            ...
            let paraLang =getUrlParameter('lang');
            window.location = path +'?lang='+ paraLang +  '&menu=temp';
...
function saveMenu() {
    ...
    request.done(function (res) {
        ...
        if (res.status === "true") {
            ...
            let path = window.location.href.split('?')[0];
            let paraLang =getUrlParameter('lang');
            window.location = path +'?lang='+ paraLang;
